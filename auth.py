...
def sign_up():
    ...
    if request.method == 'POST':
        ...
        elif class_ not in range(1,13):
            flash('Class must be 1 - 12')
        elif password != confirm_password:
            flash("Passwords don't match.", category="Error")
        elif len(password) < 7:
            flash("Password must be at least 7 characters.", category="Error")
        else:
            new_user = User(email=email, username=username, password=generate_password_hash(password, method='pbkdf2:sha256'), class_ = class_, DOB = DOB)
